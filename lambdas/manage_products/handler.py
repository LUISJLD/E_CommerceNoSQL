...
# Cliente S3 para generar URLs prefirmadas
s3_client = boto3.client(
    's3',
    endpoint_url=os.environ.get('DYNAMODB_ENDPOINT_URL'), # LocalStack endpoint
...
def _generate_upload_url(file_name, file_type):
    ...
    presigned_url = s3_client.generate_presigned_url(
        ...
        Params={
            ...
            'ACL': 'public-read' # Para que el frontend pueda leerla
        },
        ExpiresIn=3600 # 1 hora
    )
    
    # Reemplazar hostname interno de Docker por localhost para el navegador
...
def _handle_post(table, body):
    # Si viene con request de upload de imagen
...
def _handle_put(table, prod_id, body):
    ...
    # Manejar palabra reservada 'name' en Dynamo
